...
export default {
  ...
  async created() {
    if (this.id) {
      await this.carregarCurriculo();
    }
  },
  methods: {
    async carregarCurriculo() {
      try {
        const response = await axios.get(`${apiUrl}/curriculos/${this.id}`);
    ...
    async editarCurriculo() {
      try {
        await axios.put(`${apiUrl}/curriculos/${this.id}`, this.curriculo);
        alert('Currículo atualizado com sucesso!');
        this.$router.push('/curriculos');
      } catch (error) {
